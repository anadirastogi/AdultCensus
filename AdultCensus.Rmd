...
roc
```{r}
ct_roc<-roc(test$income,test$ct_ad_pred_prob,auc=TRUE)
...
```{r}
set.seed(1)
rf_model_holdout<-randomForest(income~age+workclass+fnlwgt+education+education.num+marital.status+occupation+relationship+race+sex+capital.loss+capital.gain+hours.per.week+native.country,data=mydata2,ntree=1000, cutoff=c(0.5,0.5), mtry=2,importance=TRUE)
rf_model_holdout

mydata2$rf_vote<-predict(rf_model,type="prob")[,2]
mydata2$rf_pred_class<-predict(rf_model,mydata2,type="class")
glimpse(mydata2)

table(mydata2$income==mydata2$rf_pred_class)  
```