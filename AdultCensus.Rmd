...
rf_model<-randomForest(default~age+workclass+fnlwgt+education.num+marital.status+occupation+relationship+race+sex+capital.loss+capital.gain+hours.per.week+native.country,              # model formula
                       data=Default,ntree=1000, cutoff=c(0.5,0.5))
```

```{r}
set.seed(1)
test_selection <- sample(30162, 6000) # random selection of indices.
test_rfm <- mydata2[test_selection,]       # save 20% as a test dataset
training_rfm <-mydata2[test_selection,]   # save the rest as a training set 
```
